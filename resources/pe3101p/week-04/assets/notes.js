/* PE3101P week 4 -- maths rendering and small page behaviours.
 *
 * ! KaTeX auto-render is DELIBERATELY NOT USED. These notes are full of prices
...
(function () {
  ...
  /* The .tex's own operators and shorthands, so the SAME source renders in the
     PDF and here. Each mirrors a line of the preamble:
       \DeclareMathOperator{\Cr}{Cr}     \DeclareMathOperator{\E}{E}
       \newcommand{\EU}{\mathrm{EU}}     \newcommand{\mix}[3]{#1\,#2\,#3}
     ! KaTeX 0.16.11 defines none of these -- \E in particular is NOT a KaTeX
     builtin, so there is nothing to collide with -- and it has no \nicefrac,
     which is amsmath's. Everything else the notes use (\succsim, \boldsymbol,
     \big(, \tfrac, \varnothing, \text, {,}, aligned with && columns) is native. */
  var MACROS = {
    "\\Cr": "\\operatorname{Cr}",
    "\\E": "\\operatorname{E}",
    "\\EU": "\\mathrm{EU}",
   ...
   *
   * ! THE GUTTER IS RESERVED WHETHER THE PANEL IS OPEN OR NOT, and that is his
  ...
  function peekPanel() {
    ...
    function open(key) {
      ...
      // ! Strip every id out of the copy. The panel shows a SECOND copy of a
    ...
    /* !! THE PANEL MUST NEVER SCROLL SIDEWAYS. Tomi, 22 August 2026, on Bayes'
       ...
       ! THE FIRST ATTEMPT SCALED THE BLOCK WITH `transform: scale()`. THAT DOES
       ...
       ! AND THE FIRST VERIFICATION MISSED IT, because it measured `.peekbody`'s
    ...
    /* !! HIDDEN CONTENT IS NOT MEASURED. A result's panel entry now carries its
       PROOF, inside a <details> that starts closed, and a closed details in
       Chrome still reports a scrollWidth -- a stale one, from a layout that is
       not on screen. Measuring it made fit() shrink an align* to its 40% floor
       and then, still "overflowing", drag the WHOLE definition down to 52% to
       solve a problem no reader could see. That is the same failure as the
       primer's round 7, one level deeper: not measuring the container instead
       of its children, but measuring a child that is not being displayed. */
    function shown(el) {
    ...
    function fit() {
      ...
      inner.querySelectorAll(".mathblock").forEach(function (mb) {
        mb.style.fontSize = "";
        if (!shown(mb) || mb.scrollWidth <= mb.clientWidth + 1) return;
    ...
    // Opening the proof inside the panel reveals maths that was never measured,
    // so re-fit then rather than pre-emptively at open time.
    peek.addEventListener("toggle", function () { fit(); }, true);
    ...
    /* Two kinds of link open the panel, and they are the same mechanism:
         a.xref.term[data-def]  -- a defined TERM found in the prose;
         a.xref[data-peek]      -- a RESULT cited by \lemref / \propref /
                                   \thmref, keyed by its label (lem:monotone).
       The primer's rule is that a result is always cited by name, so the link
       text already says which lemma it is; the panel shows the statement (and
       its proof toggle) without leaving the page. */
    document.addEventListener("click", function (e) {
      var a = e.target.closest("a.xref[data-def], a.xref[data-peek]");
      if (!a) return;
      var key = a.getAttribute("data-def") || a.getAttribute("data-peek");
      // If the key has no record, the href is a perfectly good ordinary link;
   ...
   *
   * ! THE PREVIOUS VERSION NEVER HIGHLIGHTED ANYTHING. It observed
  ...
  function spy() {
    ...
    // window.PRIMER does. ! Needed because the scroll handler is throttled with
   ...
   * One frame at a time, back and forward. ! Frames are kept in the DOM and
   ...
   * PDF and gets an interactive one here. ! The static SVG is only replaced
